...
var PieceItem = React.createClass({
	...
	render: function(){
		...
		for(var i in item.params){
			params.push(<div className='piece-item-param'>{Math.round(item.params[i]*100)/100}</div>)
		}
...
var Browser = React.createClass({
	...
	makeList: function(props,state,items){
		...
		for(var i = 0;i<items.length;i++){
			this.items.push(<PieceItem index = {i} ease_dur={0.5} delay={0.1}  w={Math.floor(1+Math.random()*2)} h={Math.floor(1+Math.random()*2)} color = {this.colors[props.browser_tab]} item = {items[i]} key = {props.browser_tab+'_piece_item_'+i} />)
		}
	},
	...
	render: function(){
		...
		return (
			<I beta = {this.props.beta} vertical outerClassName = 'piece_list_wrapper' scroll vertical ref="wrapper">
				...
				<G ref = "grid" offset = {this.state.list_offset} fill_up={true} fixed={true} w = {2} h={6} list_id = {this.props.browser_tab} className='piece_list' style = {{height: 'calc(100% - 50px)'}} >
					{this.items}
				</G>
